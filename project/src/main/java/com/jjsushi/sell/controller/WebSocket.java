package com.jjsushi.sell.controller;


import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import javax.websocket.OnClose;
import javax.websocket.OnMessage;
import javax.websocket.OnOpen;
import javax.websocket.Session;
import javax.websocket.server.ServerEndpoint;
import java.io.IOException;
import java.util.concurrent.CopyOnWriteArraySet;

@Component
@ServerEndpoint("/webSocket")
@Slf4j
public class WebSocket {

    private Session session;
    private static CopyOnWriteArraySet<WebSocket> webSocketsSet = new CopyOnWriteArraySet<>();
    @OnOpen
    public void onOpen(Session session){
        this.session=session;
        webSocketsSet.add(this);
        log.info("[websocket messages] new connections,{}",webSocketsSet.size());
    }
    @OnClose
    public void onClose(){
        webSocketsSet.remove(this);
        log.info("[websocket] disconnected, total:{}",webSocketsSet.size());
    }
    @OnMessage
    public void onMessage(String message){

        log.info("[websocket] got new message :{}",message);
    }

    public void sendMessage(String message) {
        for(WebSocket webSocket:webSocketsSet){
            log.info("[websocket] message={}",message);
            try {
                webSocket.session.getBasicRemote().sendText(message);

            }
            catch(Exception e){
                e.printStackTrace();
            }
        }
    }
}
